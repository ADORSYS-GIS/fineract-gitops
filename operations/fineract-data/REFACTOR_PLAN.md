...
- [ ] **Create**: `operations/fineract-data/scripts/sync-fineract-schemas.sh`
  ```bash
  #!/bin/bash
  set -e

  FINERACT_DIR="/Users/guymoyo/dev/fineract"
  GITOPS_DIR="/Users/guymoyo/dev/fineract-gitops"

  echo "📥 Extracting Fineract OpenAPI schema..."
  cd "$FINERACT_DIR"
  ./gradlew generateOpenApiDocs  # Adjust based on actual task name

  echo "📋 Copying schema to GitOps repo..."
  cp build/openapi.yaml \
    "$GITOPS_DIR/operations/fineract-data/schemas/fineract-openapi.yaml"

  echo "🔧 Generating Pydantic models..."
  cd "$GITOPS_DIR/operations/fineract-data"
  datamodel-codegen \
  ...
  echo "📊 Review changes:"
  git diff scripts/loaders/schemas_generated.py
  ```

- [ ] **Make executable**: `chmod +x scripts/sync-fineract-schemas.sh`
- [ ] **Test manually**: `./scripts/sync-fineract-schemas.sh`

### 3.2 Create GitHub Action for Weekly Sync
- [ ] **Create**: `.github/workflows/sync-fineract-schemas.yml`
  ```yaml
...
- [ ] **Test GitHub Action**: Trigger manual run via Actions tab

### 3.3 Create Schema Management Documentation
- [ ] **Create**: `operations/fineract-data/docs/SCHEMA_MANAGEMENT.md`
  - [ ] Document how schema sync works
  - [ ] How to review schema PRs
  - [ ] How to manually trigger sync
  - [ ] What to do if schema breaks compatibility
  - [ ] How to roll back schema changes
  - [ ] Migration guide for breaking changes

### 3.4 Testing
- [ ] **Test manual schema extraction**
- [ ] **Test PR creation workflow**
- [ ] **Simulate breaking change** - verify detection
- [ ] **Test schema rollback procedure**
