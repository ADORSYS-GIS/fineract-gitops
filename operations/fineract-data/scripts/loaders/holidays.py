...
"""
Holidays Loader
Loads holidays and non-working days into Fineract from YAML files
"""
import sys
import argparse
...
class HolidaysLoader(BaseLoader):
    """Loader for Fineract Holidays"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Holidays YAML to Fineract API payload
        ...
        payload = {
            ...
            'fromDate': spec.get('fromDate'),
            'toDate': spec.get('toDate'),
            'repaymentsRescheduledTo': spec.get('repaymentsRescheduledTo'),
        ...
        if 'offices' in spec:
            office_ids = []
            for office_name in spec['offices']:
                office_id = self._resolve_office(office_name)
                if office_id:
                    office_ids.append(office_id)
            if office_ids:
                payload['offices'] = office_ids
    ...
    def load_all(self) -> dict:
        """
        Load all holidays and non-working days YAML files
        ...
        for yaml_file in yaml_files:
            ...
            existing_id = self.entity_exists('holidays', entity_name)
            ...
            response = self.post('holidays', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created holidays and non-working days: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create holidays and non-working days: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Holidays into Fineract')
    ...
    loader = HolidaysLoader(args.yaml_dir, args.fineract_url, args.tenant)
