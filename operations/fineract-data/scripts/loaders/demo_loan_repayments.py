#!/usr/bin/env python3
"""
Demo Loan Repayments Loader
Loads demo loan repayments into Fineract from YAML files
"""
import sys
import argparse
...
class DemoLoanRepaymentsLoader(BaseLoader):
    """Loader for Fineract Demo Loan Repayments"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Loan Repayments YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo loan repayments YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO LOAN REPAYMENTS")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'LoanRepayment':
                logger.warning(f"  Skipping (not LoanRepayment): {yaml_file.name}")
                continue
            ...
            if response and 'resourceId' in response:
                ...
                logger.info(f"  ✓ Created demo loan repayments: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo loan repayments: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Loan Repayments into Fineract')
    ...
    loader = DemoLoanRepaymentsLoader(args.yaml_dir, args.fineract_url, args.tenant)
