...
"""
Reports Loader
Loads financial reports into Fineract from YAML files
"""
import sys
import argparse
...
class ReportsLoader(BaseLoader):
    ...
    def load_all(self) -> dict:
        """
        Load all financial reports YAML files
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'Report':
                logger.warning(f"  Skipping (not Report): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('reports', entity_name)
            ...
            response = self.post('reports', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created financial reports: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create financial reports: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Reports into Fineract')
    ...
    loader = ReportsLoader(args.yaml_dir, args.fineract_url, args.tenant)
