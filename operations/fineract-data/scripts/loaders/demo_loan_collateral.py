#!/usr/bin/env python3
"""
Demo Loan Collateral Loader
Loads demo loan collateral into Fineract from YAML files
"""
import sys
import argparse
...
class DemoLoanCollateralLoader(BaseLoader):
    """Loader for Fineract Demo Loan Collateral"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Loan Collateral YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo loan collateral YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO LOAN COLLATERAL")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'LoanCollateral':
                logger.warning(f"  Skipping (not LoanCollateral): {yaml_file.name}")
                continue
            ...
            if response and 'resourceId' in response:
                ...
                logger.info(f"  ✓ Created demo loan collateral: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo loan collateral: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Loan Collateral into Fineract')
    ...
    loader = DemoLoanCollateralLoader(args.yaml_dir, args.fineract_url, args.tenant)
