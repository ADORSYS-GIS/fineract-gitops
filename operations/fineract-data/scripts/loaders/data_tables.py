...
"""
Data Tables Loader
Loads custom data tables into Fineract from YAML files
"""
import sys
import argparse
...
class DataTablesLoader(BaseLoader):
    """Loader for Fineract Data Tables"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Data Tables YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all custom data tables YAML files
        ...
        for yaml_file in yaml_files:
            ...
            existing_id = self.entity_exists('datatables', entity_name)
            ...
            response = self.post('datatables', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created custom data tables: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create custom data tables: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Data Tables into Fineract')
    ...
    loader = DataTablesLoader(args.yaml_dir, args.fineract_url, args.tenant)
