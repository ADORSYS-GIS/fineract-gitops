#!/usr/bin/env python3
"""
Demo Loan Guarantors Loader
Loads demo loan guarantors into Fineract from YAML files
"""
import sys
import argparse
...
class DemoLoanGuarantorsLoader(BaseLoader):
    """Loader for Fineract Demo Loan Guarantors"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Loan Guarantors YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo loan guarantors YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO LOAN GUARANTORS")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'LoanGuarantor':
                logger.warning(f"  Skipping (not LoanGuarantor): {yaml_file.name}")
                continue
            ...
            if response and 'resourceId' in response:
                ...
                logger.info(f"  ✓ Created demo loan guarantors: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo loan guarantors: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Loan Guarantors into Fineract')
    ...
    loader = DemoLoanGuarantorsLoader(args.yaml_dir, args.fineract_url, args.tenant)
