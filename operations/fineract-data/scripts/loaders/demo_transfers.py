#!/usr/bin/env python3
"""
Demo Transfers Loader
Loads demo account transfers into Fineract from YAML files
"""
import sys
import argparse
...
class DemoTransfersLoader(BaseLoader):
    """Loader for Fineract Demo Transfers"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Transfers YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo account transfers YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO TRANSFERS")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'AccountTransfer':
                logger.warning(f"  Skipping (not AccountTransfer): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('accounttransfers', entity_name)
            ...
            response = self.post('accounttransfers', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created demo account transfers: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo account transfers: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Transfers into Fineract')
    ...
    loader = DemoTransfersLoader(args.yaml_dir, args.fineract_url, args.tenant)
