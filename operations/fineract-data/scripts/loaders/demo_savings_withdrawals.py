#!/usr/bin/env python3
"""
Demo Savings Withdrawals Loader
Loads demo savings withdrawals into Fineract from YAML files
"""
import sys
import argparse
...
class DemoSavingsWithdrawalsLoader(BaseLoader):
    """Loader for Fineract Demo Savings Withdrawals"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Savings Withdrawals YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo savings withdrawals YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO SAVINGS WITHDRAWALS")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'SavingsWithdrawal':
                logger.warning(f"  Skipping (not SavingsWithdrawal): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('savingsaccounts', entity_name)
            ...
            response = self.post('savingsaccounts', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created demo savings withdrawals: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo savings withdrawals: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Savings Withdrawals into Fineract')
    ...
    loader = DemoSavingsWithdrawalsLoader(args.yaml_dir, args.fineract_url, args.tenant)
