...
"""
Demo Clients Loader
Loads demo client accounts into Fineract from YAML files
"""
import sys
import argparse
...
class DemoClientsLoader(BaseLoader):
    """Loader for Fineract Demo Clients"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Clients YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo client accounts YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO CLIENTS")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'Client':
                logger.warning(f"  Skipping (not Client): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('clients', entity_name)
            ...
            response = self.post('clients', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created demo client accounts: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo client accounts: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Clients into Fineract')
    ...
    loader = DemoClientsLoader(args.yaml_dir, args.fineract_url, args.tenant)
