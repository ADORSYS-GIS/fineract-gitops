#!/usr/bin/env python3
"""
Floating Rates Loader
Loads floating interest rates into Fineract from YAML files
"""
import sys
import argparse
...
class FloatingRatesLoader(BaseLoader):
    """Loader for Fineract Floating Rates"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Floating Rates YAML to Fineract API payload
        ...
        # Build rate periods array
        rate_periods = []
        for period in spec.get('ratePeriods', []):
            rate_period = {
                'fromDate': self._format_date(period.get('fromDate')),
                'interestRate': period.get('interestRate'),
                'isDifferentialToBaseLendingRate': period.get('isDifferentialToBaseLendingRate', False),
        ...
        payload = {
            'name': spec.get('name'),
            'isBaseLendingRate': spec.get('isBaseLendingRate', False),
    ...
    def load_all(self) -> dict:
        """
        Load all floating interest rates YAML files
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'FloatingRate':
                logger.warning(f"  Skipping (not FloatingRate): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('floatingrates', entity_name)
            ...
            response = self.post('floatingrates', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created floating interest rates: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create floating interest rates: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Floating Rates into Fineract')
    ...
    loader = FloatingRatesLoader(args.yaml_dir, args.fineract_url, args.tenant)
