#!/usr/bin/env python3
"""
Guarantor Types Loader
Loads guarantor types into Fineract from YAML files
"""
import sys
import argparse
...
class GuarantorTypesLoader(BaseLoader):
    """Loader for Fineract Guarantor Types"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Guarantor Types YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all guarantor types YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING GUARANTOR TYPES")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'GuarantorType':
                logger.warning(f"  Skipping (not GuarantorType): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('guarantors', entity_name)
            ...
            response = self.post('guarantors', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created guarantor types: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create guarantor types: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Guarantor Types into Fineract')
    ...
    loader = GuarantorTypesLoader(args.yaml_dir, args.fineract_url, args.tenant)
