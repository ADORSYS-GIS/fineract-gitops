#!/usr/bin/env python3
"""
Transactions Consolidated Loader
Loads all transaction data (Waves 35-39) in sequence
"""
import sys
import argparse
import logging
...
ENTITY_LOADERS = [
    ('demo_savings_deposits', 'DemoSavingsDepositsLoader'),
    ('demo_savings_withdrawals', 'DemoSavingsWithdrawalsLoader'),
    ('demo_loan_repayments', 'DemoLoanRepaymentsLoader'),
    ('demo_transfers', 'DemoTransfersLoader'),
]
...
def main():
    parser = argparse.ArgumentParser(description='Load Transactions into Fineract (Waves 35-39)')
    ...
    entity_data_dirs = {
        'demo_savings_deposits': base_data_dir / 'transactions' / 'savings-deposits',
        'demo_savings_withdrawals': base_data_dir / 'transactions' / 'savings-withdrawals',
        'demo_loan_repayments': base_data_dir / 'transactions' / 'loan-repayments',
        'demo_transfers': base_data_dir / 'transactions' / 'inter-branch-transfers',
    }
    ...
    logger.info("\n" + "=" * 80)
    logger.info("TRANSACTIONS - FINAL SUMMARY")
    logger.info("=" * 80)
