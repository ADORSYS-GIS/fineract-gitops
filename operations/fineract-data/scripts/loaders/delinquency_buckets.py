#!/usr/bin/env python3
"""
Delinquency Buckets Loader
Loads delinquency buckets into Fineract from YAML files
"""
import sys
import argparse
...
class DelinquencyBucketsLoader(BaseLoader):
    """Loader for Fineract Delinquency Buckets"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Delinquency Buckets YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all delinquency buckets YAML files
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'DelinquencyBucket':
                logger.warning(f"  Skipping (not DelinquencyBucket): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('delinquency/buckets', entity_name)
            ...
            response = self.post('delinquency/buckets', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created delinquency buckets: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create delinquency buckets: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Delinquency Buckets into Fineract')
    ...
    loader = DelinquencyBucketsLoader(args.yaml_dir, args.fineract_url, args.tenant)
