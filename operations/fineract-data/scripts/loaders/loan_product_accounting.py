#!/usr/bin/env python3
"""
Loan Product Accounting Loader
Loads loan product GL mappings into Fineract from YAML files
"""
import sys
import argparse
...
class LoanProductAccountingLoader(BaseLoader):
    """Loader for Fineract Loan Product Accounting"""
    ...
    def __init__(self, yaml_dir: str, fineract_url: str, tenant: str = 'default'):
        ...
        self.mapping_field_map = {
            ...
            'Interest on Loans': 'interestOnLoanAccountId',
            ...
            'Overpayment Liability': 'overpaymentLiabilityAccountId',
            'Income from Charge Off Fees': 'incomeFromChargeOffFeesAccountId',
            'Income from Charge Off Penalty': 'incomeFromChargeOffPenaltyAccountId',
            'Charge Off Expense': 'chargeOffExpenseAccountId',
            'Charge Off Fraud Expense': 'chargeOffFraudExpenseAccountId',
            'Income from Goodwill Credit Fees': 'incomeFromGoodwillCreditFeesAccountId',
            'Income from Goodwill Credit Interest': 'incomeFromGoodwillCreditInterestAccountId',
            'Income from Goodwill Credit Penalty': 'incomeFromGoodwillCreditPenaltyAccountId'
        }

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Loan Product Accounting YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all loan product GL mappings YAML files
        ...
        """
        logger.info("=" * 80)
        logger.info("LOADING LOAN PRODUCT ACCOUNTING")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'LoanProductAccounting':
                logger.warning(f"  Skipping (not LoanProductAccounting): {yaml_file.name}")
                continue
            ...
            # Look up the loan product
            product_id = self._resolve_product(product_name, 'loan')

            if not product_id:
                logger.error(f"  Loan product not found: {product_name}")
            ...
            response = self.put(f'loanproducts/{product_id}', api_payload)
...
def main():
    parser = argparse.ArgumentParser(description='Load Loan Product Accounting into Fineract')
    ...
    loader = LoanProductAccountingLoader(args.yaml_dir, args.fineract_url, args.tenant)
