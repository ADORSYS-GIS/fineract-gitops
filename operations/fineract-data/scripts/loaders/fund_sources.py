...
"""
Fund Sources Loader
Loads fund sources into Fineract from YAML files
"""
import sys
import argparse
...
class FundSourcesLoader(BaseLoader):
    """Loader for Fineract Fund Sources"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Fund Sources YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all fund sources YAML files
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'FundSource':
                logger.warning(f"  Skipping (not FundSource): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('funds', entity_name)
            ...
            response = self.post('funds', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created fund sources: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create fund sources: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Fund Sources into Fineract')
    ...
    loader = FundSourcesLoader(args.yaml_dir, args.fineract_url, args.tenant)
