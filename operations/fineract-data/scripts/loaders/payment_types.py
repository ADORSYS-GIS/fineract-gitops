#!/usr/bin/env python3
"""
Payment Types Loader
Loads payment types into Fineract from YAML files
"""
import sys
import argparse
...
class PaymentTypesLoader(BaseLoader):
    """Loader for Fineract Payment Types"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Payment Types YAML to Fineract API payload
        ...
        payload = {
            ...
            'isCashPayment': spec.get('isCashPayment', False),
            'position': spec.get('position', 1),
            'locale': 'en'
        }

        # Add code value if specified
        if 'codeName' in spec:
            payload['codeName'] = spec['codeName']

        return payload

    def load_all(self) -> dict:
        """
        Load all payment types YAML files
        ...
        for yaml_file in yaml_files:
            ...
            if response and 'resourceId' in response:
                ...
                logger.info(f"  ✓ Created payment types: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create payment types: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Payment Types into Fineract')
    ...
    loader = PaymentTypesLoader(args.yaml_dir, args.fineract_url, args.tenant)
