#!/usr/bin/env python3
"""
Demo Loan Disbursements Loader
Loads demo loan disbursements into Fineract from YAML files
"""
import sys
import argparse
...
class DemoLoanDisbursementsLoader(BaseLoader):
    """Loader for Fineract Demo Loan Disbursements"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Loan Disbursements YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo loan disbursements YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO LOAN DISBURSEMENTS")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'LoanDisbursement':
                logger.warning(f"  Skipping (not LoanDisbursement): {yaml_file.name}")
                continue
            ...
            if response and 'resourceId' in response:
                ...
                logger.info(f"  ✓ Created demo loan disbursements: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo loan disbursements: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Loan Disbursements into Fineract')
    ...
    loader = DemoLoanDisbursementsLoader(args.yaml_dir, args.fineract_url, args.tenant)
