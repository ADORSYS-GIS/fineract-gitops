#!/usr/bin/env python3
"""
Loan Provisioning Loader
Loads loan provisioning criteria into Fineract from YAML files
"""
import sys
import argparse
...
class LoanProvisioningLoader(BaseLoader):
    """Loader for Fineract Loan Provisioning"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Loan Provisioning YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all loan provisioning criteria YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING LOAN PROVISIONING")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'LoanProvisioning':
                logger.warning(f"  Skipping (not LoanProvisioning): {yaml_file.name}")
                continue
            ...
            existing_id = self.entity_exists('provisioningcriteria', entity_name)
            ...
            response = self.post('provisioningcriteria', api_payload)

            if response and 'resourceId' in response:
                entity_id = response['resourceId']
                logger.info(f"  ✓ Created loan provisioning criteria: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create loan provisioning criteria: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Loan Provisioning into Fineract')
    ...
    loader = LoanProvisioningLoader(args.yaml_dir, args.fineract_url, args.tenant)
