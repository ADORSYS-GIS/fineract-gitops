#!/usr/bin/env python3
"""
Demo Savings Deposits Loader
Loads demo savings deposits into Fineract from YAML files
"""
import sys
import argparse
...
class DemoSavingsDepositsLoader(BaseLoader):
    """Loader for Fineract Demo Savings Deposits"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Savings Deposits YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo savings deposits YAML files
        ...
        for yaml_file in yaml_files:
            ...
            if response and 'resourceId' in response:
                ...
                logger.info(f"  ✓ Created demo savings deposits: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo savings deposits: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Savings Deposits into Fineract')
    ...
    loader = DemoSavingsDepositsLoader(args.yaml_dir, args.fineract_url, args.tenant)
