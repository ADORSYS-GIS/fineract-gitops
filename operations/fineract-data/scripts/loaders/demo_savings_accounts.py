#!/usr/bin/env python3
"""
Demo Savings Accounts Loader
Loads demo savings accounts into Fineract from YAML files
"""
import sys
import argparse
...
class DemoSavingsAccountsLoader(BaseLoader):
    """Loader for Fineract Demo Savings Accounts"""

    def yaml_to_fineract_api(self, yaml_data: dict) -> dict:
        """
        Convert Demo Savings Accounts YAML to Fineract API payload
    ...
    def load_all(self) -> dict:
        """
        Load all demo savings accounts YAML files

        Returns:
            Summary dict
        """
        logger.info("=" * 80)
        logger.info("LOADING DEMO SAVINGS ACCOUNTS")
        logger.info("=" * 80)
        ...
        for yaml_file in yaml_files:
            ...
            if yaml_data.get('kind') != 'SavingsAccount':
                logger.warning(f"  Skipping (not SavingsAccount): {yaml_file.name}")
                continue
            ...
            if response and 'resourceId' in response:
                ...
                logger.info(f"  ✓ Created demo savings accounts: {entity_name} (ID: {entity_id})")
                self.loaded_entities[entity_name] = entity_id
            else:
                logger.error(f"  ✗ Failed to create demo savings accounts: {entity_name}")
                self.failed_entities.append(yaml_file.name)
...
def main():
    parser = argparse.ArgumentParser(description='Load Demo Savings Accounts into Fineract')
    ...
    loader = DemoSavingsAccountsLoader(args.yaml_dir, args.fineract_url, args.tenant)
