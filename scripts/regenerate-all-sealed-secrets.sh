...
# ============================================================================
# Step 9: Commit to Git (Environment-Dependent)
# ============================================================================

log_info "Step 9: Handle Git changes..."
echo

if git diff --quiet secrets/${ENV}/; then
    log_warn "No changes to commit (sealed secrets unchanged)"
else
    # Environment-specific commit policy
    case "$ENV" in
        dev)
            log_info "Dev environment: Sealed secrets are cluster-specific (not committing)"
            echo
            echo -e "${YELLOW}Why not commit dev secrets?${NC}"
            echo "  • Dev sealed secrets are encrypted with THIS cluster's unique key"
            echo "  • Next cluster deployment will have different keys"
            echo "  • Committed secrets would fail to decrypt on other clusters"
            echo "  • Better to regenerate per-cluster for dev"
            echo
            log_info "Sealed secrets applied to cluster but not committed to Git"
            ;;

        uat|production)
            log_info "${ENV} environment: Prompting to commit (for GitOps workflow)"
            echo
            echo -e "${BLUE}Production sealed secrets should be committed IF:${NC}"
            echo "  ✓ Controller keys are backed up to AWS Secrets Manager"
            echo "  ✓ You plan to restore same keys to other ${ENV} clusters"
            echo "  ✓ You want Git as source of truth for ${ENV}"
            echo
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                ...
                git commit -m "feat: regenerate sealed secrets for ${ENV}

Regenerated sealed secrets with cluster encryption keys:
...
IMPORTANT: These secrets require the backed-up controller keys.
Restore keys before deploying: ./scripts/restore-sealed-secrets-keys.sh"
                ...
                if [[ $REPLY =~ ^[Yy]$ ]]; then
                    ...
                    log_info "Skipping push. Push manually with: git push"
                fi
            else
